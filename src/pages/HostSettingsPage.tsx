...
/** Sanitize a value for safe shell variable interpolation — strips quotes, backticks, dollar signs, and newlines */
...
function generateInstallScript(params: ScriptParams): string {
  const s = shellSafe;
  return `#!/bin/bash
# ═══════════════════════════════════════════════════════════
...
HOST_IP="${s(params.hostIp)}"
AGENT_PORT="${s(params.agentPort)}"
BINARY_PATH="${s(params.binaryPath)}"
QUERY_PATH="${s(params.queryPath)}"
DATA_DIR="${s(params.dataDir)}"
CONFIG_DIR="${s(params.configDir)}"
GAME_PORT="${s(params.gamePort)}"
...
curl -fSL "${s(params.dedicatedUrl)}" -o "\${BINARY_PATH}" || { echo "Failed to download dedicated server"; exit 1; }
curl -fSL "${s(params.queryUrl)}" -o "\${QUERY_PATH}" || { echo "Failed to download server query"; exit 1; }
...
function HostSetup() {
  ...
  const testDownload = async (url: string, name: string, setLoading: (v: boolean) => void) => {
    ...
    try {
      ...
      if (res.ok) {
        toast({ title: `${name} OK`, description: 'Binary is accessible' });
      } else {
        toast({ title: `${name} failed`, description: `HTTP ${res.status}`, variant: 'destructive' });
      }
    } catch {
      toast({ title: `${name} failed`, description: 'Could not reach storage', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const generateScript = () => {
    ...
    const script = generateInstallScript({
      hostIp, agentPort, binaryPath, queryPath, dataDir, configDir, gamePort,
      dedicatedUrl, queryUrl,
    });
    setGenerated(script);
